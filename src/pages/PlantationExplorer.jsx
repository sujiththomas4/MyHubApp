...
import {
  ...
  useZoneItems, useCare, useDefects,
  removeZoneItem, removeCare, removeDefect,
...
const TYPE_PLURAL = { land: 'properties', zone: 'zones', vertical: 'rows', pole: 'poles', plant: 'plants', zoneItem: 'items', care: 'care tasks', defect: 'defects' }
...
function TreeNode({ type, node, depth, byType, expanded, toggle, onAdd, onBulk, onEdit, onDelete, parentRef = '' }) {
  const cfg = ENTITY[type]
  const childType = cfg.childType
  const children = childType
    ? byType[childType].filter((c) => c[ENTITY[childType].parentField] === node.id).sort((a, b) => a.sortOrder - b.sortOrder)
    : []
  ...
  return (
    <>
      ...
      {isOpen && children.map((c) => (
        <TreeNode key={c.id} type={childType} node={c} depth={depth + 1} byType={byType}
          expanded={expanded} toggle={toggle} onAdd={onAdd} onBulk={onBulk} onEdit={onEdit} onDelete={onDelete} parentRef={nodeRef} />
      ))}
...
export default function PlantationExplorer() {
  ...
  const REMOVE = {
    ...
    zoneItem: removeZoneItem, care: removeCare, defect: removeDefect,
  }
  // Collect a node plus every descendant and linked record (children first).
  const collect = (type, node) => {
    const out = []
    const delPlant = (p) => { defects.filter((d) => d.plantId === p.id).forEach((d) => out.push({ type: 'defect', id: d.id })); out.push({ type: 'plant', id: p.id }) }
    const delPole = (po) => {
      plants.filter((pl) => pl.poleId === po.id).forEach(delPlant)
      care.filter((c) => c.poleId === po.id).forEach((c) => out.push({ type: 'care', id: c.id }))
      defects.filter((d) => d.poleId === po.id).forEach((d) => out.push({ type: 'defect', id: d.id }))
      out.push({ type: 'pole', id: po.id })
    }
    const delVertical = (v) => { poles.filter((po) => po.verticalId === v.id).forEach(delPole); out.push({ type: 'vertical', id: v.id }) }
    const delZone = (z) => {
    ...
    const delLand = (l) => { zones.filter((z) => z.landId === l.id).forEach(delZone); out.push({ type: 'land', id: l.id }) }
    const runner = { land: delLand, zone: delZone, vertical: delVertical, pole: delPole, plant: delPlant }
    runner[type](node)
    // de-dupe (a defect could match both a pole and its plant)
    const seen = new Set()
  ...
  const orphanDeletions = () => {
    ...
    defects.filter((d) => (d.poleId || d.plantId) && !(d.poleId && byId.pole[d.poleId]) && !(d.plantId && byId.plant[d.plantId])).forEach((d) => out.push({ type: 'defect', id: d.id }))
    const seen = new Set()
    return out.filter((d) => { const k = d.type + ':' + d.id; if (seen.has(k)) return false; seen.add(k); return true })
  }
  ...
  const rowsOf = (type) => byType[type].slice().sort((a, b) => a.sortOrder - b.sortOrder)
    ...
    <div className="option-buying">
      ...
      {lands.length === 0 && orphans.length === 0 ? (
        ...
        <div className="card mb-0"><div className="card-body p-0">
          {rowsOf('land').map((l) => (
            <TreeNode key={l.id} type="land" node={l} depth={0} byType={byType}
              expanded={expanded} toggle={toggle} onAdd={openAdd} onBulk={openBulk} onEdit={openEdit} onDelete={askDelete} />
          ))}
        </div></div>
      ) : view === 'board' ? (
        <div className="card mb-0"><div className="card-body">
          <div className="row g-3">
            {STATUSES.map((s) => {
              const items = plants.filter((p) => (p.status || 'healthy') === s.key)
              return (
