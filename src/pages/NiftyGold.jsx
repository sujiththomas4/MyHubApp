...
const TERMS = [{ value: 'long', label: 'Long term (accumulate)' }, { value: 'short', label: 'Short term (rotate)' }]
  ...
  return (
    <>
      <div className="row g-2">
        ...
        <div className="col-md-6"><label className="form-label small mb-1">Type</label>
          <select className="form-select" value={f.term} onChange={(e) => set('term', e.target.value)}>
            {TERMS.map((tm) => <option key={tm.value} value={tm.value}>{tm.label}</option>)}
          </select>
        </div>
        <div className="col-md-4"><label className="form-label small mb-1">Quantity</label><input type="number" className="form-control" value={f.qty} onChange={(e) => set('qty', e.target.value)} autoFocus /></div>
        <div className="col-md-4"><label className="form-label small mb-1">Unit price (₹)</label><input type="number" className="form-control" value={f.price} onChange={(e) => setF((s) => ({ ...s, price: e.target.value, priceTouched: true }))} /></div>
      ...
      {f.term === 'long' && <div className="text-muted small mt-2"><i className="ri-information-line me-1" />Long-term (accumulation) — not squared off; you keep buying.</div>}
