...
/**
 * CommentThread — a reusable threaded comments panel with @-mention support.
 * The parent supplies the already-filtered comments plus add/delete handlers, so
 * this works for any entity (bookings, activities, …).
 *
 * Props: open, title, comments (sorted asc), users [{value,label}],
 * currentUserId, onAdd({body,mentions}), onDelete(comment), onClose.
 */
...
function Body({ text, names }) {
  ...
  while (rest && guard < 200) {
    ...
    const hit = sorted.find((n) => n && after.toLowerCase().startsWith(n.toLowerCase()))
...
export default function CommentThread({ open, title, comments, users, currentUserId, onAdd, onDelete, onClose }) {
  const [del, setDel] = useState(null)
  const names = users.map((u) => u.label)
  const confirmDelete = async () => { const t = del; setDel(null); await onDelete(t) }

  return (
    <Modal open={open} size="lg" title={title} onClose={onClose}>
      ...
      <div className="border-top mt-3 pt-3">
        <Composer users={users} onSubmit={onAdd} />
      </div>
