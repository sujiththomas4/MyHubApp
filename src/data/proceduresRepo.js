...
// ---- Contacts --------------------------------------------------------------
const rowToContact = (r) => ({ id: r.id, name: r.name || '', role: r.role || '', phone: r.phone || '', email: r.email || '', address: r.address || '', note: r.note || '', sortOrder: r.sort_order ?? 0 })
const contactToRow = (x) => ({ id: x.id, name: x.name, role: x.role || null, phone: x.phone || null, email: x.email || null, address: x.address || null, note: x.note || null, sort_order: x.sortOrder ?? 0, updated_at: iso() })
