...
const rowTo = (r) => ({
  ...
  scheduleId: r.schedule_id || '', occurrenceDate: r.occurrence_date || '',
})
const toRow = (x) => ({
  ...
  concerns: x.concerns || null, next_plan: x.nextPlan || null, image: x.image || null, sort_order: x.sortOrder ?? 0,
  schedule_id: x.scheduleId || null, occurrence_date: x.occurrenceDate || null, updated_at: iso(),
})

export function useSupervisorUpdates() {
  const { data, loading, error, reload } = useCollection('plantation_supervisor_updates', [], { orderBy: 'period_from', ascending: false, map: rowTo })
